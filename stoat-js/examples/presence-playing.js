/**
 * Rich presence demo: "Playing Call of Duty MW2" while this process runs.
 *
 * Optional: STOAT_ACTIVITY_IMAGE=https://... (https only)
 *
 *   $env:STOAT_PRESENCE_TOKEN="stp_..."
 *   node examples/presence-playing.js
 */
...
const IMAGE = (process.env.STOAT_ACTIVITY_IMAGE || '').trim() || undefined;
...
async function main() {
  if (!TOKEN) {
    console.error('Missing token. Set STOAT_PRESENCE_TOKEN or: node examples/presence-playing.js <token>');
    process.exit(1);
  }
  ...
  console.log(`Playing "${ACTIVITY.name}" (lease ${TTL_SECONDS}s, heartbeat every ${HEARTBEAT_MS / 1000}s). Ctrl+C to stop.`);
